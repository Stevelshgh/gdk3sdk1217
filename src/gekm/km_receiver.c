...
volatile uint8_t  NKM_Tx_Flag;                                                  // UART2 TX flag
 ...
 * @fn      UART2_Rx_Service
 *
 * @brief   UART2 rx service routine that sends the data received by uart2 via USB-HID.
 *
 * @return  none
 */
...
/*********************************************************************
 * @fn      NKM_Tx_Service (UART2_Tx_Service)
 *
 * @brief   sends the data received from north port (USB2) to 
 *  south port (USB1).
 *
 * @return  none
 */
void NKM_Tx_Service( void )
{
    uint16_t pkg_len = 0;
    uint8_t *pbuf;
    if (NKM_Tx_Flag)
